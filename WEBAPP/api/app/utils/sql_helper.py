class SqlHelper:
    ...
    def to_minutes_sql(col: str) -> str:
        ...
        Times before 06:00 are shifted +1440 to preserve next-day ordering (mirrors DateTimeUtils.hhmm_to_minutes)."""
        return (
    ...
    def overlap_where_clause() -> str:
        ...
        Expects two named parameters:
          :overlap_to   → to_minutes(to_time)   (orario_inizio must be strictly less)
          :overlap_from → to_minutes(from_time) (orario_fine must be strictly greater)
        """
        inizio = SqlHelper.to_minutes_sql("orario_inizio")
        fine = SqlHelper.to_minutes_sql("orario_fine")
        return f"""
            AND ({inizio}) < :overlap_to
            AND ({fine}) > :overlap_from
        """
